...
from allennlp.data.tokenizers import Tokenizer, Token, WhitespaceTokenizer, PretrainedTransformerTokenizer
from allennlp.data.token_indexers import TokenIndexer, SingleIdTokenIndexer, PretrainedTransformerIndexer

logger = logging.getLogger(__name__)


@DatasetReader.register("semantic_parsing")
class Seq2SeqDatasetReader(DatasetReader):
    ...
    def _read(self, file_path: str):
        ...
        with open(cached_path(file_path), "r") as data_file:
            ...
            for line_num, row in self.shard_iterable(
                ...
                # For COGS, there are 3 columns
                if len(row) == 3:
                    source_sequence, target_sequence, gen_type = row
                # For other tasks, there are 2 columns
                elif len(row) == 2:
                    source_sequence, target_sequence = row
                    gen_type = None

                yield self.text_to_instance(source_sequence, target_sequence, gen_type=gen_type)
    ...
    def text_to_instance(
        ...
        if gen_type is not None:
            metadata_dict["gen_type"] = gen_type
        if target_string is not None:
            ...
            metadata_dict["target_tokens"] = [token.text for token in
                                              tokenized_target[1:-1]]
...
if __name__ == "__main__":
    ...
    max_decode_len=0
    max_source_len=0
    instances = list(datareader.read("data/cfq/mcd1/dev.tsv"))
