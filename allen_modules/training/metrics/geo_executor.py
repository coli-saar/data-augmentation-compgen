...
GEO_ARITIES = {'stateid': 1, 'place': 1, 'riverid': 1, 'lowest': 1, 'area_1': 1, 'largest': 1, 'mountain': 1,
               ...
               'capital_1': 1, 'sum': 1, 'highest': 1, 'count': 1, 'smallest': 1, 'cityid': 2, 'length': 1, 'density_1': 1,
...
class ProgramExecutorGeo:
    ...
    def eval_preds(self, pred, gold):
        gold_deno = self.execute(self.postprocess(gold))
        pred_deno = self.execute(self.postprocess(pred)) if self.postprocess(pred) else None
        if pred_deno is not None and gold_deno == pred_deno:
            return 1
        else:
            return 0

    def eval_tsv(self, pred_file):
        with open(pred_file, "r") as f:
            lines = f.readlines()
            corr, num = 0, 0
            for line in lines:
                _, gold, pred = line.rstrip().split("\t")
                gold_deno = self.execute(self.postprocess(gold))
                pred_deno = self.execute(self.postprocess(pred)) if self.postprocess(pred) else None
                if pred_deno is not None and gold_deno == pred_deno:
                    corr += 1
                else:
                    # print("gold: {} \t {} {}".format(gold, self.postprocess(gold), gold_deno))
                    # print("pred: {} \t {} {}".format(pred, self.postprocess(pred), pred_deno))
                    pass
                num += 1
            print("Execution Acc: {} with {}/{}".format(corr*1.0/num, corr, num))

    def eval_bracket_tsv(self, pred_file):
        with open(pred_file, "r") as f:
            lines = f.readlines()
            corr, num = 0, 0
            for line in lines:
                _, gold, pred = line.rstrip().split("\t")
                gold_deno = self.execute(gold)
                pred_deno = self.execute(pred) if pred else None
                if pred_deno is not None and gold_deno == pred_deno:
                    corr += 1
                else:
                    # print("gold: {} \t {} {}".format(gold, self.postprocess(gold), gold_deno))
...
@Metric.register("geo_acc")
class GeoExecuteAcc(Metric):
    def __init__(self, print_err=False):
        self.executor = ProgramExecutorGeo()
        self.match_num = 0
        self.total_num = 0
    ...
    def __call__(self, predicted_text: List[str],
        ...
        for i in range(len(predicted_text)):
            ...
            self.match_num += self.executor.eval_preds(predstr, goldstr)

            self.total_num += 1

    def get_metric(self, reset: bool) -> Dict[str, Any]:
        acc = self.match_num * 1.0 / self.total_num if self.total_num != 0 else 0
        metric_dict = {'geo_acc': acc}
        if reset:
            self.reset()
        return metric_dict
