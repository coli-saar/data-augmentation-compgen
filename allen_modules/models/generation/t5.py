...
from allen_modules.training.metrics.geo_executor import GeoExecuteAcc
...
class T5(Model):
    def __init__(
        ...
        **kwargs
        ...
        # For geoquery, we need to compute the geo executor accuracy
        self.val_geo_acc = val_geo_acc
        if self.val_geo_acc:
            self._geo_acc = GeoExecuteAcc(print_err=print_err)
            self._metrics.append(self._geo_acc)
    ...
    def forward(  # type: ignore
        ...
        if self.training:
            ...
            if labels is not None:
                ...
                if self.val_geo_acc:
                    self._geo_acc(output_dict["predicted_text"], metadata)
...
Model.register("from_pretrained_archive", constructor="from_archive")(T5)
