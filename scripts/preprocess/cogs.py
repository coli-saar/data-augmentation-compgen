...
def convert_COGS_lines(lines):
    output = []
    for line in lines:
        src, tgt, gen_type = line.rstrip().split("\t")
        if gen_type == "primitive" or len(src.split()) == 1:
            simp_tgt = src
        else:
            simp_tgt = cogs_lf_to_funcall(tgt)
        newline = "{}\t{}\t{}\n".format(src, simp_tgt, gen_type)
        output.append(newline)
    return output

def convert_corpus(dirpath):
    filename = ["train.tsv", "dev.tsv",  "test.tsv", "gen.tsv"]
    for f in filename:
        filepath = "{}/{}".format(dirpath, f)
        lines = read_file(filepath)
        write_file(filepath, convert_COGS_lines(lines))
        print("Converted {} lines in {}".format(len(lines), filepath))

if __name__ == "__main__":
    convert_corpus("data/cogs/")